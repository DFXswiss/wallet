...
const getAllTokens = async (client: WhaleApiClient): Promise<TokenData[]> => {
  const allTokens: TokenData[] = await getPaginatedResponse<TokenData>((limit, next) =>
    client.tokens.list(limit, next),
  );
...
export const fetchTokens = createAsyncThunk(
  ...
  async ({
    ...
    address: string;
    client: WhaleApiClient;
  }): Promise<{ tokens: AddressToken[]; allTokens: TokenData[]; utxoBalance: string }> => {
...
export const wallet = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchPoolPairs.fulfilled, (state, action: PayloadAction<DexItem[]>) => {
      ...
      state.poolpairs = action.payload.filter(({ data }) => !data.symbol.includes('/v1')); // Filter out v1 pairs due to stock split
    });
    builder.addCase(
      fetchDexPrice.fulfilled,
      (state, action: PayloadAction<{ dexPrices: DexPricesProps; denomination: string }>) => {
        state.dexPrices = {
          ...state.dexPrices,
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<{ tokens: AddressToken[]; allTokens: TokenData[]; utxoBalance: string }>) => {
