...
export const setUserPreferences = createAsyncThunk(
  ...
  async ({ network, preferences }: { network: EnvironmentNetwork; preferences: UserPreferences }) => {
    await LocalStorageProvider.setUserPreferences(network, preferences);
  },
);

export const setAddresses = createAsyncThunk('userPreferences/setAddresses', async (addresses: LabeledAddress) => {
  return addresses;
});
