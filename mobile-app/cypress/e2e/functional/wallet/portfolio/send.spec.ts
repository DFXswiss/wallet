...
context('Wallet - Send', function () {
  ...
  describe('DFI UTXO', function () {
                    ...
                    .then((textFeeValue) => {
                      ...
                      cy.getByTestID('resulting_DFI')
                        ...
                        .then((pendingBalanceValue) => {
                          ...
                          expect(
                            new BigNumber(balance).plus(slippage).minus(transactionFee).minus(sendAmount).toFixed(8),
                          ).eq(pendingBalance);
    ...
    it('should be able to transfer correct amount when user cancel a tx and updated some inputs', function () {
      ...
      cy.getByTestID('txn_authorization_description').contains(`Sending ${new BigNumber(oldAmount).toFixed(8)} DFI`);
      ...
      cy.getByTestID('button_confirm_send').click().wait(3000);
      // Check for authorization page description
      cy.getByTestID('txn_authorization_description').contains(`Sending ${new BigNumber(newAmount).toFixed(8)} DFI`);
      cy.closeOceanInterface();
    });
  });
  ...
  addresses.forEach(function (address) {
    it(`should be able to send to address ${address}`, function () {
      ...
      cy.getByTestID('transaction_details_info_text').should(
        'contain',
        'Review full transaction details in the next screen',
      );
      ...
      cy.getByTestID('txn_authorization_description').contains(
        `Converting ${new BigNumber('2.1').toFixed(8)} Token to UTXO`,
      );
