context('Wallet - Token Detail', () => {
  beforeEach(function () {
    ...
    cy.sendDFItoWallet().sendDFITokentoWallet().sendTokenToWallet(['BTC']).wait(10000);
...
context('Wallet - Token Detail - LP', () => {
  beforeEach(function () {
    cy.createEmptyWallet(true);
    cy.getByTestID('header_settings').click();
    cy.sendDFItoWallet().sendDFITokentoWallet().sendTokenToWallet(['ETH-DFI', 'ETH']).wait(10000);
    cy.getByTestID('bottom_tab_portfolio').click();
    cy.getByTestID('portfolio_list').should('exist');
