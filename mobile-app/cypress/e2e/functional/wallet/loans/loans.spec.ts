...
import { checkCollateralDetailValues, checkVaultDetailValues } from '../../../../support/loanCommands';
...
function sendTokenToRandomAddress(tokenId: string, isMax = false): void {
  ...
  isMax ? cy.getByTestID('MAX_amount_button').click() : cy.getByTestID('amount_input').clear().type('10').blur();
          ...
          cy.getByTestID('payback_input_text')
            ...
            .then((text) => {
              checkValueWithinRange(
                text,
                outstandingBalance.multipliedBy(loanToPaymentTokenRate).multipliedBy(penaltyFee).div(2).toFixed(8),
                1,
              );
            });
        }
        ...
        .then((text) => {
          ...
          cy.getByTestID('payback_input_text')
            ...
            .then((text) => {
              checkValueWithinRange(
                text,
                outstandingBalance.multipliedBy(loanToPaymentTokenRate).multipliedBy(penaltyFee).toFixed(8),
                1,
              );
            });
        });
...
function borrowFirstLoan(loanTokenSymbol: string, amount: string = '10'): void {
  ...
  cy.getByTestID('txn_authorization_description').contains(`Borrowing ${amountToBorrow} ${loanTokenSymbol}`);
  cy.closeOceanInterface();
}
...
context('Wallet - Loans - Take Loans', () => {
  ...
  it('should add loan', function () {
    ...
    cy.getByTestID('text_resulting_col_ratio').contains('1,500.00');
    cy.getByTestID('button_confirm_borrow_loan').click().wait(3000);
    cy.getByTestID('txn_authorization_description').contains('Borrowing 100.00000000 DUSD');
    cy.closeOceanInterface();
  });
  ...
  it('should verify collaterals page', function () {
    cy.getByTestID('vault_card_0_edit_collaterals_button').click();
    checkCollateralDetailValues('ACTIVE', '$1,500.00', '$748.00', 201.0, '%', '150.00', '5.00');
  });
...
context('Wallet - Loans Payback Non-DUSD Loans', () => {
  ...
  it('should borrow dTU10 loan', function () {
    ...
    cy.getByTestID('button_confirm_borrow_loan').click().wait(3000);
    cy.getByTestID('txn_authorization_description').contains('Borrowing 10.00000000 dTU10');
    cy.closeOceanInterface();
  });
  ...
  it('should be able to payback dTU10 loans with dTU10', function () {
    ...
    cy.getByTestID('text_current_collateral_ratio').contains('N/A');
    cy.getByTestID('button_confirm_payback_loan').click().wait(4000);
    cy.getByTestID('txn_authorization_description').contains('Paying 12.00000000 dTU10');
    cy.closeOceanInterface();
    cy.wait(3000);
  ...
  it('should be able to partially payback dTU10 loans with DUSD', function () {
    ...
    cy.getByTestID('text_vault_id').contains(vaultId);
    cy.getByTestID('button_confirm_payback_loan').click().wait(4000);
    cy.getByTestID('txn_authorization_description').contains('Paying 20.00000000 DUSD');
    cy.closeOceanInterface();
    cy.wait(3000);
    cy.checkVaultTag('ACTIVE', VaultStatus.Healthy, 'vault_card_0_status', walletTheme.isDark);
  });
