...
context('Wallet - Loans - Create vault', () => {
  ...
  it('should display correct loan schemes in create vault screen', function () {
    ...
    cy.wait(['@loanSchemes']).then((intercept: any) => {
      ...
      data
        .sort((a, b) => new BigNumber(a.minColRatio).minus(b.minColRatio).toNumber())
...
context('Wallet - Loans - Confirm create vault', () => {
  ...
  it('should display tooltip message for oracle pricing', () => {
    ...
    cy.getByTestID('icon-tooltip-text')
      .should('exist')
      .should('have.text', 'This icon indicates that the price is provided by Oracles instead of the DEX');
