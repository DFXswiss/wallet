...
context('Wallet - Loans - Health Bar', () => {
  ...
  it('should calculate next ratio using next price from oracle', function () {
    ...
    const nextRatioValue = new BigNumber(150.0).dividedBy(loanInNextPrice).multipliedBy(100).toFixed(2);
    cy.getByTestID('vault_card_0_next_ratio')
      .invoke('text')
      .then(function (nextRatio: string) {
        expect(nextRatio).to.equal(`~${nextRatioValue}%`);
      });
    cy.getByTestID('vault_card_0_col_ratio')
      .invoke('text')
      .then(function (colRatio: string) {
