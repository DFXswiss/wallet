...
function addCollateral(
  ...
  cy.getByTestID('txn_authorization_description').contains(`Adding ${precisedAmount} ${token} as collateral`);
...
function removeCollateral(
  ...
  cy.getByTestID('button_confirm_confirm_edit_collateral').click().wait(3000);
  cy.getByTestID('txn_authorization_description').contains(`Removing ${precisedAmount} ${token} collateral from vault`);
  cy.closeOceanInterface();
}
...
function borrowLoan(symbol: string, amount: string): void {
  ...
  cy.getByTestID('txn_authorization_description').contains(`Borrowing ${amountToBorrow} ${symbol}`);
  cy.closeOceanInterface();
}
  ...
  it('should go to collateral page', function () {
    ...
    cy.wait(['@loanCollaterals']).then((intercept: any) => {
      ...
      data.forEach((collateralToken: CollateralToken) => {
        cy.getByTestID(`token_symbol_${collateralToken.token.displaySymbol}`).contains(
          collateralToken.token.displaySymbol,
        );
        cy.getByTestID(`select_${collateralToken.token.displaySymbol}_value`).contains(
          amounts[collateralToken.token.displaySymbol] ?? 0,
        );
      });
    });
  });
