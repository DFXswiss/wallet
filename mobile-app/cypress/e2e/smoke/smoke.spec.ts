...
context('Mainnet - Wallet', () => {
  ...
  it('should store values of local wallet', function () {
    ...
    cy.sendDFItoWallet().sendDFITokentoWallet().sendTokenToWallet(['BTC', 'ETH-DFI']).wait(3000);
  ...
  it('should verify poolpair values', function () {
    ...
    cy.wrap<DexItem[]>(whale.poolpairs.list(5), { timeout: 20000 }).then((pairs) => {
      ...
      available.forEach((pair, index) => {
        ...
        cy.getByTestID(`available_${data.symbol}_${data.tokenA.displaySymbol}`).contains(
          `${new BigNumber(new BigNumber(data.tokenA.reserve).toFixed(2, 1)).toNumber().toLocaleString()}`,
        );
        cy.getByTestID(`available_${data.symbol}_${data.tokenB.displaySymbol}`).contains(
          `${new BigNumber(new BigNumber(data.tokenB.reserve).toFixed(2, 1)).toNumber().toLocaleString()}`,
        );
