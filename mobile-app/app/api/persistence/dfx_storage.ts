...
enum StoreItem {
  DFXWALLET_ADDRESS_LIST = 'DFXWALLET.address_list',
  DFXWALLET_PIN = 'DFXWALLET.pin',
  USERINFOCOMPLETE = 'DFXWALLET.userInfoComplete',
  DFXWALLET_BACKUP = 'DFXWALLET.backup',
}
...
async function setWalletPin(pin: string): Promise<void> {
  await AsyncStorage.setItem(StoreItem.DFXWALLET_PIN, pin);
}

// Get wallet pin
async function getWalletPin(): Promise<string> {
  return (await AsyncStorage.getItem(StoreItem.DFXWALLET_PIN)) ?? '';
}

// Get/set the pair list
async function getPairList(): Promise<DFXAddrSignature[]> {
  const listStr: string = (await AsyncStorage.getItem(StoreItem.DFXWALLET_ADDRESS_LIST)) ?? '[]';
  return JSON.parse(listStr);
}

async function setPairList(list: DFXAddrSignature[]): Promise<void> {
  return await AsyncStorage.setItem(StoreItem.DFXWALLET_ADDRESS_LIST, JSON.stringify(list));
}
...
async function reset(network: string): Promise<void> {
  ...
  await AsyncStorage.setItem(StoreItem.DFXWALLET_PIN, '');
}
...
async function setUserInfoComplete(address: string, complete = true): Promise<void> {
  await AsyncStorage.setItem(StoreItem.USERINFOCOMPLETE + address, JSON.stringify(complete));
}

async function getUserInfoComplete(address: string): Promise<boolean | null> {
  const status = await AsyncStorage.getItem(StoreItem.USERINFOCOMPLETE + address);
