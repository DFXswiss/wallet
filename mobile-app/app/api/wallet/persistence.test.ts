...
describe('WalletPersistence.get()', () => {
  ...
  it('should get empty', async () => {
    getItem.mockResolvedValueOnce(EnvironmentNetwork.LocalPlayground).mockResolvedValueOnce('0');
  ...
  it('should get non empty', async () => {
    ...
    expect(items).toStrictEqual([{ raw: 'raw-1', type: WalletType.MNEMONIC_UNPROTECTED, version: 'v1' }]);
  });
});
...
describe('WalletPersistence.set()', () => {
  ...
  it('should set 1 wallet, clear 0 wallet', async () => {
    ...
    await WalletPersistence.set([{ raw: 'raw-1', type: WalletType.MNEMONIC_UNPROTECTED, version: 'v1' }]);
