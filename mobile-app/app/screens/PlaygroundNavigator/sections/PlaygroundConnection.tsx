...
export function PlaygroundConnection(): JSX.Element {
  ...
  return (
    <View>
      <PlaygroundTitle status={{ online: connected, offline: !connected }} title="Connection" />

      <ThemedView dark={tailwind('bg-dfxblue-800')} light={tailwind('bg-white')} style={tailwind('px-4 py-4')}>
        <Text>
          <ThemedText style={tailwind('font-medium')}>Network:</ThemedText>

          <ThemedText testID="playground_active_network"> {network}</ThemedText>
        </Text>

        <ThemedText>Blocks: {count === 0 ? '...' : <Text testID="current_block_count">{count}</Text>}</ThemedText>
