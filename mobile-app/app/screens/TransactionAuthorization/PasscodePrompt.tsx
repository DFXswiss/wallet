      ...
      >
        {props.status === TransactionStatus.AUTHORIZED ? (
          ...
          <ThemedView light={tailwind('bg-white')} dark={tailwind('bg-dfxblue-800')}>
            <ThemedText style={tailwind('text-center text-xl font-bold px-1')}>{props.title}</ThemedText>
            ...
            <Loading message={props.status === TransactionStatus.SIGNING ? props.loadingMessage : undefined} />
...
export const PasscodePrompt = React.memo((props: PasscodePromptProps): JSX.Element => {
  ...
  if (Platform.OS === 'web') {
    return (
      <SafeAreaView style={tailwind('w-full h-full flex-col absolute z-50 top-0 left-0')} ref={containerRef}>
        <Modal
  ...
  return (
      ...
      backgroundComponent={(backgroundProps: BottomSheetBackgroundProps) => (
        <View
          ...
          style={[
            ...
            tailwind(`${isLight ? 'bg-white border-gray-200' : 'bg-dfxblue-900 border-dfxblue-900'} border-t rounded`),
          ]}
        />
      )}
      onChange={(index) => {
    ...
    >
      <SafeAreaView style={tailwind('w-full h-full flex-col absolute z-50 top-0 left-0')}>
        <PromptContent {...props} />
      </SafeAreaView>
    </BottomSheetModal>
  );
