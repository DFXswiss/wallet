...
export function ConfirmPoolSwapScreen({ route }: Props): JSX.Element {
  const { tokenA, tokenB, fee, swap, pair, slippage, priceRateA, priceRateB } = route.params;
  ...
  return (
    <ThemedScrollView style={tailwind('pb-4')}>
        ...
        rhs={{
          value:
            conversion?.isConversionRequired === true
              ? translate('screens/PoolSwapConfirmScreen', 'Convert & swap')
      ...
      />
      <FeeInfoRow type="ESTIMATED_FEE" value={fee.toFixed(8)} testID="text_fee" suffix="DFI" />
...
async function constructSignedSwapAndSend(
  ...
  try {
    const maxPrice = dexForm.fromAmount
      .div(dexForm.toAmount)
      .times(1 + slippage)
      .decimalPlaces(8);
