  ...
  return (
    <ThemedScrollView style={tailwind('w-full flex-col flex-1')}>
      <ThemedView style={tailwind('w-full mt-6')}>
        <ThemedText style={tailwind('text-base ml-4 mb-2')}>
          {translate('screens/RemoveLiquidity', 'Drag or enter amount to remove')}
        </ThemedText>
        <AmountSlider current={Number(percentage)} onChange={setInputPercentage} viewStyle={tailwind('p-4')} />
        <ThemedView
      ...
      <ThemedSectionTitle testID="remove_liq_title" text={translate('screens/RemoveLiquidity', 'YOU ARE REMOVING')} />
      <TokenBalanceRow
      ...
      <ContinueButton enabled={valid} onPress={removeLiquidity} />
    </ThemedScrollView>
  );
}
...
function AmountSlider(props: {
  ...
  return (
    ...
    >
      <TouchableOpacity onPress={() => props.onChange('0.00')} testID="button_slider_min">
        <ThemedText
          dark={tailwind('text-dfxgray-300')}
      ...
      <TouchableOpacity onPress={() => props.onChange('100.00')} testID="button_slider_max">
        <ThemedText
...
function HelperText(props: { displayedPercentage: string }): JSX.Element {
  return (
    <ThemedView light={tailwind('bg-transparent')} dark={tailwind('bg-transparent')} style={tailwind('flex-row mt-2')}>
      <ThemedText style={tailwind('text-sm')}>{`${translate(
