...
export function useDexStabilization (tokenA: DexStabilizationTokenA, tokenB: DexStabilizationTokenB): {
  ...
  const _getHighDexStabilizationFeeAnnouncement = useCallback(async (tokenA: DexStabilizationTokenA, tokenB: DexStabilizationTokenB): Promise<AnnouncementData[]> => {
    ...
    if (dexStabilizationType === 'direct-dusd-with-fee') {
      ...
      announcement = [{
        lang: {
          en: `When swapping into dToken assets, please note that if you want to exchange them back into crypto tokens (DFI, dBTC, dETH...), stablecoins (dUSDC, dUSDT) or sell them via DFX against FIAT (EUR, USD, FR...) at a later point in time, a DEX stabilization fee of roughly 12% (currently ${fee}%) will apply. In case of a later exchange within the dToken (dUSD, dTSLA, dAMZN, dCOIN ...), the aforementioned DEX stabilization fee does not apply.`,
          de: `Beim Tausch in dToken-Vermögenswerte beachte bitte, dass bei einem späteren Rücktausch in Krypto-Token (DFI, dBTC, dETH...), Stablecoins (dUSDC, dUSDT) oder einem Verkauf über DFX gegen FIAT (EUR, USD, FR...) eine DEX-Stabilisierungsgebühr von etwa 12% (derzeit ${fee}%) anfällt. Bei einem späteren Umtausch innerhalb des dToken (dUSD, dTSLA, dAMZN, dCOIN ...) entfällt die vorgenannte DEX-Stabilisierungsgebühr.`,
          ...
          fr: `Lors de l'échange contre des actifs dToken, veuillez noter que si vous souhaitez les échanger à nouveau contre des crypto-tokens (DFI, dBTC, dETH...), des stablecoins (dUSDC, dUSDT) ou les vendre via DFX contre des FIAT (EUR, USD, FR...) ultérieurement, des frais de stabilisation DEX d'environ 12% (actuellement ${fee}%) s'appliqueront. En cas d'échange ultérieur au sein du dToken (dUSD, dTSLA, dAMZN, dCOIN ...), les frais de stabilisation DEX susmentionnés ne s'appliquent pas.`,
          es: `Cuando cambie a activos dToken, tenga en cuenta que si quiere volver a cambiarlos por cripto tokens (DFI, dBTC, dETH...), stablecoins (dUSDC, dUSDT) o venderlos a través de DFX contra FIAT (EUR, USD, FR...) en un momento posterior, se aplicará una tasa de estabilización DEX de aproximadamente el 12% (actualmente ${fee}%). En caso de un intercambio posterior dentro del dToken (dUSD, dTSLA, dAMZN, dCOIN ...), no se aplica la mencionada tasa de estabilización DEX.`,
          it: `Quando si scambiano asset in dToken, si noti che se si desidera scambiarli nuovamente in crypto token (DFI, dBTC, dETH...), stablecoin (dUSDC, dUSDT) o venderli tramite DFX contro FIAT (EUR, USD, FR...) in un momento successivo, si applicherà una commissione di stabilizzazione DEX di circa il 12% (attualmente ${fee}%). In caso di scambio successivo all'interno del dToken (dUSD, dTSLA, dAMZN, dCOIN...), la suddetta commissione di stabilizzazione DEX non si applica.`
        },
        version: '>=1.16.1',
        url: {
          ios: highFeesUrl,
  ...
  const _getDexStabilization = async (tokenA: DexStabilizationTokenA, tokenB: DexStabilizationTokenB): Promise<DexStabilization> => {
    ...
    */
    ...
    if (compositeInverted != null) {
      ...
      compositeInverted.dexStabilizationFee = getDexStabilizationFee('DUSD', 'DFI')
    }
