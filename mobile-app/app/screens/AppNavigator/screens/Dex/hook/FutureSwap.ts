...
export function useFutureSwap(props: SwapType): {
  ...
  const fromTokenDetail = useSelector((state: RootState) =>
    tokenSelectorByDisplaySymbol(state.wallet, props.fromTokenDisplaySymbol ?? ''),
  );
  ...
  const isFutureSwapEnabled =
    isFeatureAvailable('future_swap') && fromTokenDetail !== undefined && toTokenDetail !== undefined;
