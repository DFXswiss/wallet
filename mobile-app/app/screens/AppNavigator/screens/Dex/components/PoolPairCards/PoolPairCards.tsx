...
export function PoolPairCards({
  ...
  useEffect(() => {
    ...
    if (debouncedSearchTerm !== undefined && debouncedSearchTerm.trim().length > 0) {
      setFilteredYourPairs(
        yourPairs
          .filter((pair) => pair.data.displaySymbol.toLowerCase().includes(debouncedSearchTerm.trim().toLowerCase()))
          .sort(pairSortingFn),
      );
    } else {
      setFilteredYourPairs([]);
  ...
  const renderItem = ({ item, index }: { item: DexItem<WalletToken | PoolPairData>; index: number }): JSX.Element => (
    <PoolCard
      index={index}
  ...
  return (
    <ThemedFlatList
      ...
      ListHeaderComponent={
        <>
          {type === 'available' && buttonGroupOptions !== undefined && showSearchInput === false && (
            <>
...
const PoolCard = ({
  ...
  const poolPairData = pairs.find((pr) => pr.data.symbol === (yourPair as AddressToken).symbol);
  ...
  const getPriceRates = async (): Promise<void> => {
    if (mappedPair !== undefined) {
      const priceRates = await calculatePriceRates(mappedPair.tokenA.id, mappedPair.tokenB.id, new BigNumber('1'));
  ...
  const toRemove = new BigNumber(1).times((yourPair as WalletToken).amount).decimalPlaces(8, BigNumber.ROUND_DOWN);
  ...
  const onCollapseToggle = (): void => {
    if (isExpanded) {
      setExpandedCardIds(expandedCardIds.filter((id) => id !== yourPair.id));
    } else {
  ...
  return (
    ...
    >
      <View style={tailwind('flex flex-row justify-between w-full')} testID={`pool_pair_row_${index}_${symbol}`}>
        <View style={tailwind('max-w-4/5 flex-shrink')}>
          <View style={tailwind('flex-row items-center')}>
            <PoolPairTextSection symbolA={symbolA} symbolB={symbolB} />
            {type === 'available' && (
              <TouchableOpacity
              ...
              >
                <ThemedIcon
                  ...
                  dark={tailwind(isFavouritePair ? 'text-dfxyellow-500' : 'text-dfxgray-300')}
                />
              </TouchableOpacity>
            )}
          </View>
        ...
        <View>
          {mappedPair?.apr?.total !== undefined && mappedPair?.apr?.total !== null && (
            <APRSection
              ...
              value={{
                text: new BigNumber(isNaN(mappedPair.apr.total) ? 0 : mappedPair.apr.total).times(100).toFixed(2),
                decimalScale: 2,
                testID: `apr_${symbol}`,
                suffix: '%',
              }}
      ...
      {type === 'available' ? (
        ...
        <View style={tailwind('flex flex-col mt-2')}>
          ...
          />
          <ActiveUSDValue
            price={getTokenPrice(yourPair.symbol, new BigNumber((yourPair as WalletToken).amount), true)}
            testId={`share_in_pool_${symbol}_USD`}
          />
        </View>
      )}
      <View style={tailwind('flex flex-row justify-between items-center mt-4 -mb-2 flex-wrap')}>
      ...
      {isExpanded && (
        <View>
          ...
          <InfoSection
            type={type}
            pair={mappedPair}
            tokenATotal={type === 'your' ? tokenATotal.toFixed(8) : mappedPair?.tokenA.reserve}
            tokenBTotal={type === 'your' ? tokenBTotal.toFixed(8) : mappedPair?.tokenB.reserve}
            testID={type}
          />
        </View>
      )}
