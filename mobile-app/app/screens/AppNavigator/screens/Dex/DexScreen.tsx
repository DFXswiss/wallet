...
export function DexScreen(): JSX.Element {
  ...
  const { poolpairs: pairs, hasFetchedPoolpairData } = useSelector((state: RootState) => state.wallet);
    ...
    debounce((searchString: string) => {
      ...
      if (searchString !== undefined && searchString.trim().length > 0) {
        setFilteredAvailablePairs(
          pairs
            .filter((pair) => pair.data.displaySymbol.toLowerCase().includes(searchString.trim().toLowerCase()))
            .sort(
              (firstPair, secondPair) =>
                new BigNumber(secondPair.data.totalLiquidity.usd ?? 0)
                  .minus(firstPair.data.totalLiquidity.usd ?? 0)
                  .toNumber() ?? new BigNumber(secondPair.data.id).minus(firstPair.data.id).toNumber(),
            ),
        );
      } else {
        setFilteredAvailablePairs([]);
  ...
  if (!isLoaded) {
    return <ThemedScrollView dark={tailwind('bg-dfxblue-900')} light={tailwind('bg-white')} />;
  }
  ...
  return (
    <>
      ...
      <View style={tailwind('flex-1')}>
        ...
        {activeTab === TabKey.YourPoolPair && yourLPTokens.length === 0 && <EmptyActivePoolpair />}
