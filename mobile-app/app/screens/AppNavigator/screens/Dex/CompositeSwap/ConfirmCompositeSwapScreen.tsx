...
export function ConfirmCompositeSwapScreen({ route }: Props): JSX.Element {
  const { conversion, fee, pairs, priceRates, slippage, tokenA, tokenB, swap, futureSwap, estimatedAmount } =
    route.params;
  ...
  return (
    <ThemedScrollView style={tailwind('pb-4')}>
      ...
      <InfoRow type={InfoType.EstimatedFee} value={fee.toFixed(8)} testID="confirm_text_fee" suffix="DFI" />
      {!isFutureSwap && (
        <>
          <NumberRow
      ...
      {futureSwap !== undefined ? (
        <>
          ...
          />
          <TextRow
            lhs={translate('screens/ConfirmCompositeSwapScreen', 'Resulting {{token}}', {
          ...
          tokens={[
            {
              symbol: tokenA.displaySymbol,
              value: BigNumber.max(
                new BigNumber(tokenA.amount).minus(swap.amountFrom).minus(tokenA.displaySymbol === 'DFI' ? fee : 0),
                0,
              ).toFixed(8),
              suffix: tokenA.displaySymbol,
            },
