...
export function CompositeSwapScreen({ route }: Props): JSX.Element {
  ...
  const {
    ...
    dexStabilization: { dexStabilizationType, pair: dexStabilizationPair, dexStabilizationFee },
    ...
    setBottomSheetScreen([
      {
        ...
        component: BottomSheetTokenList({
          ...
          headerLabel: translate(
            ...
            direction === 'FROM' ? 'Choose token for swap' : 'Choose token to swap',
          ),
  ...
  useEffect(() => {
    ...
    const tokenSelectOption: DexParamList['CompositeSwapScreen']['tokenSelectOption'] = route.params
      .tokenSelectOption ?? {
      from: {
        isDisabled: true,
        isPreselected: true,
          ...
          {selectedTokenA !== undefined && selectedTokenB !== undefined && (
            <View style={tailwind('mx-4 flex-1')}>
              ...
              <View style={tailwind(['flex flex-row items-center', { 'mb-4': isConversionRequired }])}>
                <TouchableOpacity onPress={onTokenSwitch} testID="switch_button">
                  <ThemedIcon
                    name="swap-vert"
                    size={24}
          ...
          tokenB !== undefined && (
            <>
              {!isFutureSwap && (
                ...
                >
                  <PricesSection testID="pricerate_value" priceRates={priceRates} isCompact />
                </ThemedView>
              )}
          ...
          <View style={tailwind('mb-2')}>
            <SubmitButtonGroup
              ...
              onSubmit={
                ...
                (dexStabilizationType === 'return-path-with-fee' && isDexStabilizationEnabled) ||
...
function TokenRow(form: TokenForm): JSX.Element {
  ...
  return (
    <Controller
      ...
      name="tokenA"
      render={({ field: { value } }) => (
        <ThemedView dark={tailwind('bg-transparent')} light={tailwind('bg-transparent')} style={tailwind('flex-row')}>
          <WalletTextInput
            autoCapitalize="none"
            onChange={(e) => {
...
function OraclePriceRow({ oraclePriceText, tokenDisplaySymbol }: OraclePriceRowProps): JSX.Element {
