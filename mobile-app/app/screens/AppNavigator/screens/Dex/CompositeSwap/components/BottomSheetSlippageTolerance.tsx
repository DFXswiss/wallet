...
export const BottomSheetSlippageTolerance = ({
  ...
  slippage,
  onCloseButtonPress,
}: BottomSheetSlippageToleranceProps): React.MemoExoticComponent<() => JSX.Element> =>
  memo(() => {
  ...
  return (
    <>
      ...
      <View testID="slippage_tolerance_description" style={tailwind('flex-row mb-3 items-center')}>
        <ThemedText style={tailwind('text-base')} dark={tailwind('text-gray-200')} light={tailwind('text-gray-700')}>
          {translate(
            'screens/SlippageTolerance',
            'Slippages are rate changes that occur within an order transaction. Choose how much of this slippage are you willing to accept.',
          )}
        </ThemedText>
      </View>
    </>
