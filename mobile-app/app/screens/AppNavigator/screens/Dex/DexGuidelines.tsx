    ...
    >
      ...
      >
        {translate(
          ...
          'Participate in supplying liquidity to power the DEX (Decentralized Exchange). Use your tokens to earn high returns (of up to 100%).',
        )}
      </ThemedText>

      {guidelines.map((g, i) => (
        <View key={i} style={tailwind('flex-row items-center my-4')}>
          <ThemedIcon iconType="MaterialIcons" name={g.icon} size={32} />

          <View style={tailwind('flex-col flex-auto ml-6')}>
            <ThemedText style={tailwind('font-medium')}>{translate('screens/DexGuidelines', g.title)}</ThemedText>
