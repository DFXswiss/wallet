    ...
    LOCKpostKyc()
      .then(async (kycData) => {
        if (newKyc) {
          ...
          return await transferKyc(getEnvironment(getReleaseChannel()).lock.walletName).then(() => {
            setKycComplete();
      ...
      <ScrollView contentContainerStyle={tailwind('flex-grow flex-col p-8')}>
              ...
              <View style={tailwind('flex-row pb-2 mb-4 border-b border-gray-200')}>
                ...
                <TouchableOpacity onPress={toggleHandover} style={tailwind('flex-col')}>
                  ...
                  <Text style={tailwind('flex self-center text-base mr-8')}>
                    {translate(
                      'LOCK/LockKycScreen',
                      'Transfer your KYC (Know-your-customer) data from DFX.swiss to LOCK and start staking right away.',
                    )}
                  </Text>
                </TouchableOpacity>
              </View>
              ...
              <View style={tailwind('flex-row pb-2')}>
                ...
                <TouchableOpacity onPress={toggleNew} style={tailwind('flex-col')}>
                  ...
                  <Text style={tailwind('flex self-center text-base mr-8')}>
                    {translate(
                      'LOCK/LockKycScreen',
                      'New KYC (Know-your-customer) via LOCK (operated by MN IT Services LLC based on St. Vincent and the Grenadines).',
                    )}
                  </Text>
                </TouchableOpacity>
              </View>
          ...
          <>
            <View style={tailwind('bg-white rounded-md p-2 mb-8')}>
              <View style={tailwind('flex-col pb-2')}>
                <Text style={tailwind('text-lg font-bold')}>{translate('LOCK/LockKycScreen', 'KYC Process')}</Text>
                <Text style={tailwind('flex self-center text-base mr-8')}>
                  {translate(
                    'LOCK/LockKycScreen',
                    'KYC (Know-your-customer) via LOCK (operated by MN IT Services LLC based on St. Vincent and the Grenadines).',
                  )}
                </Text>
              </View>
            </View>
        ...
        >
          ...
          <Text style={tailwind('text-xs font-medium underline mr-8')}>
            {translate(
              'LOCK/LockKycScreen',
              'I take note that by pressing “{{buttonText}}” I agree with the terms and conditions of MN IT Services LLC.',
              { buttonText },
            )}
          </Text>
        </TouchableOpacity>
