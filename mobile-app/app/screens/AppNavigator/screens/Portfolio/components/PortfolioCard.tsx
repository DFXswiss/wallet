...
function PortfolioItemRow({
  ...
  const lockedToken = (useTokenLockedBalance({
    ...
    denominationCurrency,
  }) as LockedBalance) ?? { amount: new BigNumber(0), tokenValue: new BigNumber(0) };
  ...
  const hasLockedBalance = useMemo((): boolean => {
    return (
      collateralTokens.some((collateralToken) => collateralToken.token.displaySymbol === token.displaySymbol) ||
      loanTokens.some((loanToken) => loanToken.token.displaySymbol === token.displaySymbol)
    );
  }, [token]);

  return (
    <ThemedView dark={tailwind('bg-dfxblue-800')} light={tailwind('bg-white')} style={tailwind('p-4 rounded-lg')}>
