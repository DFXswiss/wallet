...
export const CreateOrEditAddressLabelForm = memo(({ route, navigation }: Props): JSX.Element => {
  ...
  const { title, isAddressBook, address, addressLabel, onSaveButtonPress } = route.params;
  ...
  const handleEditSubmit = (): void => {
    if (labelInput === undefined || address === undefined || !validateLabelInput(labelInput)) {
      return;
    }
    onSaveButtonPress({
      [address]: {
  ...
  const isSaveDisabled = (): boolean => {
    ...
    if (
      !isAddressBook &&
      (labelInput === undefined || labelInput === addressLabel?.label || labelInputErrorMessage !== '')
    ) {
      return true;
    }
...
function AddressDisplay({ address }: { address: string }): JSX.Element {
  return (
    <View style={tailwind('flex flex-row mb-4 items-center')}>
      ...
      <ThemedText style={tailwind('text-sm ml-2 flex-1', { 'w-10/12': Platform.OS === 'web' })}>{address}</ThemedText>
    </View>
  );
}
