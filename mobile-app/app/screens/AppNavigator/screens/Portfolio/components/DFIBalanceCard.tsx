...
export function DFIBalanceCard({ denominationCurrency }: DFIBalaceCardProps): JSX.Element {
  ...
  const lockedToken = (useTokenLockedBalance({ displaySymbol: 'DFI', denominationCurrency }) as LockedBalance) ?? {
...
function DFIBreakdownPercentageItem({
  label,
  value,
  type,
}: {
  ...
  return (
    ...
    >
      <ThemedText style={tailwind('text-xs')} light={tailwind('text-gray-500')} dark={tailwind('text-dfxgray-400')}>
...
function DFIBreakdownAction({
  ...
  return (
    <View style={tailwind('flex-row justify-center flex-1 items-center')}>
      ...
      <TouchableOpacity onPress={onBreakdownPress} testID="details_dfi">
        <ThemedIcon
...
function GetDFIBtn(): JSX.Element {
  const navigation = useNavigation<NavigationProp<PortfolioParamList>>();
  return (
    <ThemedView light={tailwind('bg-primary-50')} dark={tailwind('bg-darkprimary-50')} style={tailwind('mt-1')}>
      <LinearGradient
        start={[0, 0]}
        end={[1, 1]}
        colors={[
        ...
        locations={[
          0, 0.1124, 0.2038, 0.2776, 0.3368, 0.3848, 0.4247, 0.4598, 0.4933, 0.5284, 0.5683, 0.6163, 0.6756, 0.7493,
          0.8408, 0.9531,
        ]}
      >
        <TouchableOpacity
        ...
        >
          <View style={tailwind('mx-4 my-2 flex-row justify-between flex-1 items-center')}>
            <View style={tailwind('flex-row flex-1 items-center')}>
              <Text style={tailwind('font-medium text-sm text-gray-50')}>
