...
export function PortfolioScreen({ navigation }: Props): JSX.Element {
  const { isNotAllowedInCountry, LOCKisNotAllowedInCountry, getUnavailableServices, hasVerifiedBackup } =
    useDFXAPIContext();
  const { totalBalance } = useLockStakingContext();
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerLeft: (): JSX.Element => <HeaderSettingButton />,
  ...
  const { totalAvailableValue, dstTokens } = useMemo(() => {
    return tokens.reduce(
      (
        { totalAvailableValue, dstTokens }: { totalAvailableValue: BigNumber; dstTokens: PortfolioRowToken[] },
        token,
      ) => {
  ...
  const portfolioButtonGroup = [
    {
      ...
      label: translate('screens/TotalPortfolio', 'USDT'),
  ...
  const totalLoansValue = useMemo(() => {
    ...
    return vaults.reduce((totalLoansValue: BigNumber, vault: LoanVaultActive) => {
  ...
  const expandModal = useCallback((isSortBottomSheet: boolean) => {
    if (Platform.OS === 'web') {
      ...
      isSortBottomSheet ? bottomSheetSortRef.current?.present() : bottomSheetRef.current?.present();
    }
  }, []);
  const dismissModal = useCallback((isSortBottomSheet: boolean) => {
    if (Platform.OS === 'web') {
      ...
      isSortBottomSheet ? bottomSheetSortRef.current?.close() : bottomSheetRef.current?.close();
    }
  }, []);
...
function FutureSwapCta({ navigation }: { navigation: StackNavigationProp<PortfolioParamList> }): JSX.Element {
  return (
...
function AssetSortRow(props: {
  ...
  const highestCurrencyValue = translate('screens/PortfolioScreen', 'Highest {{modifiedDenominationCurrency}} value', {
    modifiedDenominationCurrency: props.modifiedDenominationCurrency,
  });
  const lowestCurrencyValue = translate('screens/PortfolioScreen', 'Lowest {{modifiedDenominationCurrency}} value', {
    modifiedDenominationCurrency: props.modifiedDenominationCurrency,
  });
  ...
  return (
    <View style={tailwind('px-4 flex flex-row justify-between pt-5')} testID="toggle_sorting_assets">
      <ThemedText
