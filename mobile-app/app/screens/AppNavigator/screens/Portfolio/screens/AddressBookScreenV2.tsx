...
export function AddressBookScreenV2({ route, navigation }: Props): JSX.Element {
            ...
            {item.isMine ? (
              ...
              >
                {item.isFavourite === true ? (
                  <FavoriteCheckIcon size={24} testID={`address_row_${index}_is_favourite_${testIDSuffix}`} />
                ) : (
                  <FavoriteUnCheckIcon size={24} testID={`address_row_${index}_not_favourite_${testIDSuffix}`} />
                )}
              </TouchableOpacity>
            )}
            <TouchableOpacity
  ...
  return (
    <ThemedViewV2 style={tailwind('h-full')}>
      ...
      <ScrollView contentContainerStyle={tailwind('pb-8')} style={tailwind('px-5 h-full')}>
          ...
          <View style={tailwind('flex-1')}>
            <SearchInputV2
              ...
              containerStyle={[
                ...
                tailwind(
                  isSearchFocus
                    ? { 'border-mono-light-v2-800': isLight, 'border-mono-dark-v2-800': !isLight }
                    : { 'border-mono-light-v2-00': isLight, 'border-mono-dark-v2-00': !isLight },
                ),
              ]}
          ...
          <View style={tailwind('px-5 mt-8 mb-2')}>
            ...
            >
              {searchString.trim().length > 0
                ? translate('screens/AddressBookScreen', 'Search results for “{{input}}”', {
        ...
        {activeButtonGroup === ButtonGroupTabKey.Whitelisted && filteredAddressBook.length === 0 && !isSearchFocus ? (
          ...
          <>
            ...
            {(!isSearchFocus || (isSearchFocus && searchString.trim().length > 0)) &&
              (activeButtonGroup === ButtonGroupTabKey.Whitelisted ? filteredAddressBook : filteredWalletAddress).map(
                (item: LocalAddress, index: number) => (
                  <AddressListItem
                    item={item}
                    key={item.address}
                    index={index}
                    testIDSuffix={
                      activeButtonGroup === ButtonGroupTabKey.Whitelisted ? 'address_book' : 'wallet_address'
                    }
                  />
                ),
              )}
...
function EmptyDisplay({ onPress }: { onPress: () => void }): JSX.Element {
  const { isLight } = useThemeContext();
  return (
    <View style={tailwind('px-5 text-center mt-10')} testID="empty_address_book">
      <View style={tailwind('items-center pb-8')}>
        <Image style={{ width: 200, height: 136 }} source={isLight ? LightEmptyAddress : DarkEmptyAddress} />
      </View>
      <ThemedTextV2 testID="empty_tokens_title" style={tailwind('text-xl font-semibold-v2 text-center')}>
        {translate('screens/AddressBookScreen', 'No saved addresses')}
      </ThemedTextV2>
      <ThemedTextV2 testID="empty_tokens_subtitle" style={tailwind('font-normal-v2 text-center mt-2')}>
