  ...
  return (
    <ThemedScrollView>
      <View style={tailwind('flex flex-row mt-3 mx-2')}>
        <TokenSelection label={translate('screens/WithdrawFutureSwapScreen', 'FROM')} symbol={source.displaySymbol} />
        <TokenSelection
      ...
      >
        ...
        {props.symbol !== undefined && (
          <>
            ...
            <ThemedText style={tailwind('ml-2')} dark={tailwind('text-dfxgray-400')} light={tailwind('text-gray-500')}>
              {props.symbol}
            </ThemedText>
          </>
        )}
...
function AmountSlider(props: {
  ...
  return (
    <ThemedView dark={tailwind('bg-dfxblue-800')} light={tailwind('bg-white')} style={props.viewStyle}>
      <Slider
      ...
      <View style={tailwind('flex flex-row justify-between mt-1')}>
        <ThemedText style={tailwind('text-xs')} dark={tailwind('text-dfxgray-400')} light={tailwind('text-gray-500')}>
          0
        </ThemedText>
        <ThemedText style={tailwind('text-xs')} dark={tailwind('text-dfxgray-400')} light={tailwind('text-gray-500')}>
...
function HelperText(props: { displayedValue: string }): JSX.Element {
  return (
    <ThemedView light={tailwind('bg-transparent')} dark={tailwind('bg-transparent')} style={tailwind('flex-row mt-2')}>
      <ThemedText style={tailwind('text-sm')} dark={tailwind('text-dfxgray-400')} light={tailwind('text-gray-500')}>
...
export function TokenIcon(props: { icon: string }): JSX.Element {
