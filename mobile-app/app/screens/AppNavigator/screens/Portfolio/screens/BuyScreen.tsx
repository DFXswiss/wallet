...
export function BuyScreen({ route, navigation }: Props): JSX.Element {
  ...
  useEffect(() => {
    ...
    const prom1 = getAssets()
      .then((assets) => {
        assets = assets.filter((asset) => asset.buyable && asset.blockchain === 'DeFiChain');
        setAssets(assets);
      })
      .catch(logger.error);
...
function AmountRow({ control, onAmountChange, onClearButtonPress }: AmountForm): JSX.Element {
