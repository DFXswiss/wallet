...
export function AddressBookScreen({ route, navigation }: Props): JSX.Element {
  ...
  const walletAddressFromStore: LocalAddress[] = useSelector((state: RootState) =>
    selectLocalWalletAddressArray(state.userPreferences),
  ...
  const [favouriteAddress, setFavouriteAddress] = useState(
    addressBook.filter((address) => address.isFavourite === true),
  );
  ...
  const filterAddress = useCallback(
    ...
    [addressBook, walletAddress, activeButtonGroup],
  );
        ...
        return !isSearchFocus ? (
          ...
          >
            <ThemedIcon
              ...
              light={tailwind([
                'text-primary-500',
                { 'text-gray-300': activeButtonGroup === ButtonGroupTabKey.YourAddress },
              ])}
              dark={tailwind([
                'text-dfxred-500',
                { 'text-gray-600': activeButtonGroup === ButtonGroupTabKey.YourAddress },
              ])}
  ...
  const AddressListItem = useCallback(
    ({ item, index, testIDSuffix }: { item: LocalAddress; index: number; testIDSuffix: string }): JSX.Element => {
      return (
        <ThemedTouchableOpacity
          key={item.address}
              ...
              !item.isMine && (
                ...
                >
                  <ThemedIcon
                    ...
                    size={20}
                    light={tailwind(item.isFavourite === true ? 'text-warning-500' : 'text-gray-600')}
                    dark={tailwind(item.isFavourite === true ? 'text-dfxyellow-500' : 'text-dfxgray-300')}
                    testID={
                      item.isFavourite === true
                        ? `address_row_${index}_is_favourite_${testIDSuffix}`
                        : `address_row_${index}_not_favourite_${testIDSuffix}`
                    }
                  />
                </TouchableOpacity>
              )
      ...
      <ThemedView style={tailwind('flex-1')}>
        <View style={tailwind('pt-6 px-4')}>
          {searchString.trim().length === 0 ? (
            ...
            <SearchResultTextWithCounter
              searchString={searchString}
              length={filteredAddressBook.length + filteredWalletAddress.length}
            />
          )}
        </View>
        ...
        {searchString.trim().length > 0 && (
          <ScrollView contentContainerStyle={tailwind('pb-8')}>
            ...
            {filteredAddressBook.map((item: LocalAddress, index: number) => (
              <AddressListItem item={item} key={index} index={index} testIDSuffix="address_book" />
            ))}
            {filteredWalletAddress.length !== 0 && (
              <ThemedSectionTitle text={translate('screens/AddressBookScreen', 'YOUR ADDRESS(ES)')} />
            )}
            {filteredWalletAddress.map((item: LocalAddress, index: number) => (
              <AddressListItem item={item} key={index} index={index} testIDSuffix="wallet_address" />
            ))}
          </ScrollView>
        )}
      </ThemedView>
  ...
  return (
    <ThemedFlatList
      ...
      ListEmptyComponent={
        activeButtonGroup === ButtonGroupTabKey.Whitelisted && filteredAddressBook.length === 0 ? (
...
function EmptyDisplay({ onPress }: { onPress: () => void }): JSX.Element {
  ...
  return (
    <ThemedView style={tailwind('px-8 pt-24 pb-2 text-center')} testID="empty_address_book">
      <View style={tailwind('items-center pb-4')}>{isLight ? <NoTokensLight /> : <DfxNoTokensImage />}</View>
...
function SearchResultTextWithCounter({ searchString, length }: { searchString: string; length: number }): JSX.Element {
  return (
    <View style={tailwind('justify-between flex-row items-center')}>
