...
export function TokensVsUtxoScreen(): JSX.Element {
  return (
    <ThemedScrollView contentContainerStyle={tailwind('p-6 pb-8')} testID="token_vs_utxo_screen">
      <View style={tailwind('pb-6')}>
        ...
        <ThemedText light={tailwind('text-gray-700')} dark={tailwind('text-gray-200')}>
          {translate(
      ...
      <FaqInfoSection
        ...
        tokenUnit="_UTXO"
        useCases={['Sending DFI to other wallets', 'Receiving DFI from other wallets', 'Transaction fees']}
      />
      <FaqInfoSection
        ...
        tokenUnit="DFI"
        useCases={['Swapping DFI with other tokens (e.g. dBTC, dETH, dUSDT)', 'Liquidity pools', 'Auctions and Loans']}
      />
    </ThemedScrollView>
  );
}
  ...
  return (
    <View style={tailwind('mb-6')}>
      ...
      <View style={tailwind('mb-2')}>
        <ThemedText light={tailwind('text-gray-700')} dark={tailwind('text-gray-200')} style={tailwind('text-sm')}>
          {translate('screens/TokensVsUtxoScreen', 'Use cases include:')}
        </ThemedText>
      </View>
      {props.useCases.map((label) => (
        <UseCaseRow key={label} label={label} />
      ))}
    </View>
  );
}
