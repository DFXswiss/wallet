...
export function SendConfirmationScreen({ route }: Props): JSX.Element {
  ...
  const { token, destination, amount, fee, conversion } = route.params;
  ...
  const expectedBalance = BigNumber.maximum(new BigNumber(token.amount).minus(amount.toFixed(8)).minus(fee), 0).toFixed(
    8,
  );
  ...
  return (
    <ThemedScrollView style={tailwind('pb-4')}>
        ...
        rhs={{
          value:
            conversion?.isConversionRequired === true
              ? translate('screens/SendConfirmationScreen', 'Convert & send')
      ...
      {token.isLPS && <LpAcknowledgeSwitch isAcknowledge={isAcknowledge} onSwitch={(val) => setIsAcknowledge(val)} />}
...
function LpAcknowledgeSwitch(props: { isAcknowledge: boolean; onSwitch: (val: boolean) => void }): JSX.Element {
  return (
    <View style={tailwind('mx-4 mt-8 flex flex-row items-center')}>
      <Switch value={props.isAcknowledge} onValueChange={props.onSwitch} testID="lp_ack_switch" />
      <ThemedText
      ...
      >
        {translate(
          ...
          'Send Liquidity Pool tokens only to DeFiChain Wallet addresses. Sending to an exchange (except DFX) or a central entity will result in irreversible loss of funds.',
        )}
      </ThemedText>
    </View>
  );
