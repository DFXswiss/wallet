...
export function ReceiveDTokenScreen({ route, navigation }: Props): JSX.Element {
  const logger = useLogger();
  const { isLight } = useThemeContext();
  const { address } = useWalletContext();
  const [activeButton, setActiveButton] = useState<CryptoButtonGroupTabKey>(
    route.params?.crypto ?? CryptoButtonGroupTabKey.DFI,
  );
          ...
          <HeaderTitle
            text={
              activeButton === CryptoButtonGroupTabKey.DFI
                ? translate('screens/ReceiveScreen', 'Receive')
                : translate('screens/ReceiveDTokenScreen', 'Bitcoin Deposit')
            }
  ...
  return (
    <ThemedScrollView style={tailwind('px-8 pt-2 flex flex-1 w-full relative')} testID="receive_screen">
        ...
        route.params?.crypto === undefined ? (
          <View style={tailwind('pt-2 self-center')}>
            <ButtonGroup
              ...
              customActiveStyle={{
                dark: tailwind(
                  activeButton === CryptoButtonGroupTabKey.DFI ? 'bg-dfxdfi-500' : 'bg-dfxbtc-500',
                  'h-10',
                ),
              }}
            />
          ...
          <>
            ...
            >
              {activeButton === CryptoButtonGroupTabKey.DFI
                ...
                : 'BTC ' + translate('screens/ReceiveDTokenScreen', 'DEPOSIT ADDRESS')}
            </ThemedTextBasic>
            <ThemedText
      ...
      {(bitcoinAddress !== '' || activeButton === CryptoButtonGroupTabKey.DFI) && (
        <>
          <ThemedView style={tailwind('flex flex-row mt-6 mb-4')}>
            <TouchableOpacity
              onPress={() => {
                copyToClipboard();
          ...
          {activeButton === CryptoButtonGroupTabKey.BTC && (
            <>
              <InfoText
                ...
                text={translate(
                  'screens/ReceiveDTokenScreen',
                  'Please note the MINIMUM deposit amount is {{MinimumBtcAmount}}!',
                  { MinimumBtcAmount },
                )}
