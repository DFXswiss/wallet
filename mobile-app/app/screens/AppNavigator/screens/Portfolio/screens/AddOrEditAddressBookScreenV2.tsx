...
export function AddOrEditAddressBookScreenV2({ route, navigation }: Props): JSX.Element {
  ...
  const handleSubmit = useCallback(async (): Promise<void> => {
    ...
    const auth: Authentication<string[]> = {
      ...
      title: translate(
        ...
        isAddNew ? 'Add address to address book?\n{{address}}' : 'Update address label for\n{{address}}',
        { address: addressInput },
      ),
      message: translate('screens/Settings', 'Enter passcode to continue'),
      loading: translate(
        'screens/AddOrEditAddressBookScreen',
        isAddNew ? 'It may take a few seconds to save' : 'It may take a few seconds to update',
      ),
  ...
  return (
    <ThemedScrollViewV2 contentContainerStyle={tailwind('px-5 pb-16')} style={tailwind('flex-1')}>
      {isAddNew ? (
        <WalletTextInputV2
          value={addressInput}
          autoCapitalize="none"
...
function CopyAddressComponent(props: { address: string }): JSX.Element {
  ...
  return (
    ...
    >
      <ThemedSectionTitleV2 text={translate('screens/AddOrEditAddressBookScreen', 'ADDRESS')} />
      <ThemedViewV2
      ...
      >
        <View style={tailwind('flex flex-row items-center py-4.5 px-5 justify-between')}>
          <TouchableOpacity
            onPress={async () => {
              copyToClipboard();
