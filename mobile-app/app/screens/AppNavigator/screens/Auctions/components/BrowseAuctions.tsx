      ...
      {hasFetchAuctionsData ? (
        <>
          {filteredAuctionBatches.length === 0 ? (
            ...
            <BatchCards auctionBatches={filteredAuctionBatches} onQuickBid={onQuickBid} vaults={vaults} />
          )}
        </>
      ) : (
...
function BatchCards({
  ...
  const RenderItems = useCallback(({ item, index }: { item: AuctionBatchProps; index: number }): JSX.Element => {
