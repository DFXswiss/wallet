...
export const QuickBid = ({
  ...
  memo(() => {
    ...
    const { hasPendingJob, hasPendingBroadcastJob, constructSignedBidAndSend } = useSignBidAndSend();
    ...
    return (
      <ThemedView light={tailwind('bg-white')} dark={tailwind('bg-dfxblue-800')} style={tailwind('h-full flex')}>
  ...
  return (
    <View style={tailwind('flex flex-row items-center mb-7')}>
      ...
      <View style={tailwind('ml-2')}>
        <ThemedText style={tailwind('text-lg font-medium')}>{translate('components/QuickBid', 'Quick bid')}</ThemedText>
        <ThemedText light={tailwind('text-gray-700')} dark={tailwind('text-gray-200')} style={tailwind('text-xs')}>
