...
export const BottomSheetAuctionedCollateral = ({
  ...
  headerLabel,
  onCloseButtonPress,
}: BottomSheetAuctionedCollateralProps): React.MemoExoticComponent<() => JSX.Element> =>
  memo(() => {
    ...
    return (
      <FlatList
        data={collaterals}
        renderItem={({ item }: { item: LoanVaultTokenAmount }): JSX.Element => <CollateralTokenItemRow token={item} />}
        ListHeaderComponent={
          <ThemedView
