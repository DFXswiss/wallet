...
export function BidHistory(props: BidHistoryProps): JSX.Element {
  ...
  return (
    <ThemedFlatList
      ...
      ListEmptyComponent={() => (
        <EmptyBidHistory minNextBid={props.minNextBidInToken} displaySymbol={props.loanDisplaySymbol} />
      )}
      keyExtractor={(item: VaultAuctionBatchHistory) => item.id}
      contentContainerStyle={[
        tailwind('p-4 pb-8'),
        tailwind({ 'pb-44': Platform.OS !== 'android' && Platform.OS !== 'ios' }),
      ]}
...
function BidHistoryItem(props: BidHistoryItemProps): JSX.Element {
  ...
  return (
    <ThemedView
      ...
      dark={tailwind([
        'border-dfxblue-800',
        { 'bg-dfxblue-800': props.isLatestBid, 'bg-dfxblue-900': !props.isLatestBid },
      ])}
    ...
    >
      <View style={tailwind('flex flex-row justify-between mb-2 items-center')}>
        ...
        >
          <ThemedText style={tailwind('text-2xs')} light={tailwind('text-white')} dark={tailwind('text-black')}>
            {translate('components/BidHistory', 'BID #{{bidIndex}}', { bidIndex: props.bidIndex })}
          </ThemedText>
        </ThemedView>
        <ThemedText light={tailwind('text-gray-500')} dark={tailwind('text-dfxgray-400')} style={tailwind('text-xs')}>
          {bidTime}
        </ThemedText>
      </View>
      <View style={tailwind('flex flex-row justify-between items-center')}>
        <NumberFormat
          ...
          renderText={(value) => (
            ...
            >
              {value}
              <ThemedText style={tailwind('text-xs')}>{` ${props.loanDisplaySymbol}`}</ThemedText>
            </ThemedText>
          )}
        />
        <ThemedText numberOfLines={1} ellipsizeMode="middle" style={tailwind('w-4/12 text-xs font-medium')}>
          {props.bidderAddress}
        </ThemedText>
      </View>
