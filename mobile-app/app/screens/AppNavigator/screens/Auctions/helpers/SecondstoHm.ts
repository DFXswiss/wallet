...
export function secondsToHmDisplay(d: number): string {
  ...
  const mDisplay =
    m >= 0 ? translate('components/BatchCard', '{{m}}m', { m: h > 0 ? padStart(m.toString(), 2, '0') : m }) : '';
...
export function secondsToDhmsDisplay(s: number): string {
  ...
  const hDisplay =
    h > 0 ? translate('components/BatchCard', ' {{h}}h', { h: d > 0 ? padStart(h.toString(), 2, '0') : h }) : '';
  const mDisplay =
    m > 0 ? translate('components/BatchCard', ' {{m}}m', { m: h > 0 ? padStart(m.toString(), 2, '0') : m }) : '';
  const sDisplay =
    s > 0 ? translate('components/BatchCard', ' {{s}}s', { s: m > 0 ? padStart(m.toString(), 2, '0') : s }) : '';
