...
export function AuctionDetailScreen(props: BatchDetailScreenProps): JSX.Element {
  ...
  const { minNextBidInToken, totalCollateralsValueInUSD, minNextBidInUSD } = useAuctionBidValue(
    batch,
  ...
  return (
    <View style={tailwind('flex-1')} ref={containerRef}>
      <ThemedView light={tailwind('bg-gray-50')} style={tailwind(['flex-1', { 'pb-36': Platform.OS !== 'web' }])}>
        <ThemedView
            ...
            <View style={tailwind('flex flex-row items-center')}>
              ...
              <View style={tailwind('flex flex-row ml-2')}>
                <View style={tailwind('flex')}>
                  <ThemedText style={tailwind('font-semibold text-sm')}>{batch.loan.displaySymbol}</ThemedText>
                  <View style={tailwind('flex flex-row items-center')}>
                    <ThemedText
        ...
        {activeTab === TabKey.Collaterals && (
          <AuctionedCollaterals collaterals={batch.collaterals} auctionAmount={totalCollateralsValueInUSD} />
        )}

        {activeTab === TabKey.AuctionDetails && <AuctionDetails vault={vault} batch={batch} />}
      </ThemedView>
...
const AuctionActionSection = memo((props: AuctionActionSectionProps): JSX.Element => {
  return (
    ...
    >
      ...
      />
      <View style={tailwind('flex flex-row mt-4 items-center justify-center')}>
        <IconButton
