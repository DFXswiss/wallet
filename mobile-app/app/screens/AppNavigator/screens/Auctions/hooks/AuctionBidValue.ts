...
export function useAuctionBidValue(batch: LoanVaultLiquidationBatch, liquidationPenalty: number): AuctionBid {
  ...
  const minStartingBidInUSD =
    loan.activePrice?.active != null
      ? getPrecisedTokenValue(minStartingBidInToken.times(loan.activePrice.active.amount))
      : '';
  const minNextBidInToken =
    highestBid?.amount?.amount != null ? new BigNumber(highestBid.amount.amount).times(1.01) : minStartingBidInToken;
  const minNextBidInUSD = getPrecisedTokenValue(getTokenPrice(batch.loan.symbol, minNextBidInToken));
  const totalCollateralsValueInUSD = getPrecisedTokenValue(
    batch.collaterals.reduce((total, eachItem) => {
