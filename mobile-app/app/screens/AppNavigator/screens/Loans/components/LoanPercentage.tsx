  ...
  return (
    ...
    >
      ...
      <Progress.Bar
        progress={
          amountToPayInPaymentToken.gt(0)
            ? Number(amountToPayInPaymentToken.dividedBy(outstandingBalanceInPaymentToken))
            : 0
        }
        width={null}
      ...
      <View style={tailwind('flex flex-row justify-between items-center mt-2')}>
        <ThemedText style={tailwind('text-2xs')} light={tailwind('text-gray-500')} dark={tailwind('text-dfxgray-400')}>
          {translate('screens/PaybackLoanScreen', '0%')}
        </ThemedText>
        {amountToPayInPaymentToken.gt(0) && (
          <ThemedText style={tailwind('text-xs')} light={tailwind('text-gray-900')} dark={tailwind('text-gray-50')}>
            {paymentTokenDisplaySymbol !== loanTokenDisplaySymbol
              ? `${amountToPayInPaymentToken.toFixed(
                  8,
                )} ${paymentTokenDisplaySymbol} ≈ ${amountToPayInLoanToken.toFixed(8)} ${loanTokenDisplaySymbol}`
              : `${amountToPayInPaymentToken.toFixed(
                  8,
                )} ${loanTokenDisplaySymbol} / ${loanTokenOutstandingBalance.toFixed(8)}`}
          </ThemedText>
        )}
        <ThemedText style={tailwind('text-2xs')} light={tailwind('text-gray-500')} dark={tailwind('text-dfxgray-400')}>
