...
export function CollateralizationRatioValue(
  props: CollateralizationRatioProps & { value: string; type: 'current' | 'next'; testId: string },
): JSX.Element {
    ...
    <NumberFormat
      ...
      renderText={(val: string) => (
        <View style={tailwind('flex flex-row items-center flex-1 flex-wrap justify-end')}>
          <ThemedText dark={dark} light={light} style={tailwind('text-sm text-right mr-1')} testID={props.testId}>
            {props.type === 'next' && '~'}
            {val}
          </ThemedText>
        </View>
