...
export const AddOrRemoveCollateralForm = memo(({ route }: Props): JSX.Element => {
  ...
  const totalCalculatedCollateralValue = isAdd
    ? new BigNumber(totalCollateralVaultValue).plus(initialPrices?.collateralPrice)
    : new BigNumber(totalCollateralVaultValue).minus(initialPrices.collateralPrice);
  ...
  const hasInvalidColRatio =
    resultingColRatio.isLessThanOrEqualTo(0) || resultingColRatio.isNaN() || !resultingColRatio.isFinite();
  ...
  return (
    ...
    >
      ...
      <View style={tailwind('flex flex-row items-center mb-2')}>
        <SymbolIcon symbol={token.displaySymbol} styleProps={tailwind('w-6 h-6')} />
        <ThemedText testID={`token_symbol_${token.displaySymbol}`} style={tailwind('mx-2')}>
          {token.displaySymbol}
        </ThemedText>
        <ThemedView
      ...
      >
        ...
        >
          <SetAmountButton amount={new BigNumber(available)} onPress={onAmountChange} type={AmountButtonTypes.half} />

          <SetAmountButton amount={new BigNumber(available)} onPress={onAmountChange} type={AmountButtonTypes.max} />
        </ThemedView>
      </WalletTextInput>
      ...
      {isFeatureAvailable('dusd_vault_share') ? (
        <View style={tailwind('flex justify-between items-center flex-row w-full')}>
          <ThemedText style={tailwind('mr-2 w-6/12')}>
            {translate('components/AddOrRemoveCollateralForm', 'Vault requirement')}
          </ThemedText>

          <ThemedView style={tailwind('flex flex-row items-center mb-0 p-1 rounded-2xl')}>
            <TokenIconGroup
        ...
        >
          ...
          <ThemedView style={tailwind('flex flex-row items-center mb-0 py-1 px-1.5 rounded-2xl')}>
