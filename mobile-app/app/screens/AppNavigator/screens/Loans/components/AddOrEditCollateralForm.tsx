...
export const AddOrEditCollateralForm = React.memo(({ route }: Props): JSX.Element => {
  const { token, collateralFactor, available, current, onButtonPress } = route.params;
  ...
  return (
    <ThemedView light={tailwind('bg-white')} dark={tailwind('bg-dfxblue-800')} style={tailwind('p-4 flex-1')}>
      ...
      <View style={tailwind('flex flex-row items-center mb-2')}>
        ...
        <ThemedText style={tailwind('mx-2')}>{token}</ThemedText>
        <ThemedView
