...
export const useLoanPaymentTokenRate = (props: {
  ...
  const getPaymentPenalty = (paymentTokenSymbol: string): BigNumber => {
    const hasPenalty =
      paymentTokenSymbol === 'DFI' || (paymentTokenSymbol === 'DUSD' && props.loanToken.symbol !== 'DUSD');
    if (!hasPenalty) {
  ...
  const getPaymentTokens = (): GetAmountProps => {
    const paymentTokenAmounts = paymentTokens.map((paymentToken) => {
      ...
      return {
        ...
        cappedAmount:
          props.loanToken.symbol !== paymentToken.tokenSymbol ? cappedAmountInPaymentToken : cappedAmountInLoanToken,
