  ...
  return new BigNumber(collateralValue)
    .dividedBy(
      new BigNumber(existingLoanValue).plus(
        newLoanAmount.multipliedBy(interestPerBlock.plus(1)).multipliedBy(activePrice),
      ),
    )
    .multipliedBy(100);
}
...
export function getCollateralPrice(
  ...
  const vaultShare = getVaultShare(
    ...
    collateralFactor,
    activePrice,
    new BigNumber(totalCollateralValue.isZero() ? collateralPrice : totalCollateralValue),
  );
  return {
    activePrice,
    collateralPrice,
    ...
    if (collateralItem !== undefined) {
      ...
      const price =
        collateralItem !== undefined
          ? getCollateralPrice(new BigNumber(amount ?? 0), collateralItem, new BigNumber(totalCollateralVaultValue))
