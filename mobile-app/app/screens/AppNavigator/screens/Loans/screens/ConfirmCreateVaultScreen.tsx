...
function SummaryTransactionDetails(props: { fee: BigNumber; conversion?: ConversionParam }): JSX.Element {
  ...
  const vaultFee = new BigNumber(
    network === EnvironmentNetwork.MainNet || network === EnvironmentNetwork.TestNet ? 2 : 1,
  );
  const transactionCost = vaultFee.plus(props.fee);
  return (
    <>
      <ThemedSectionTitle text={translate('screens/ConfirmCreateVaultScreen', 'TRANSACTION DETAILS')} />
      <TextRow
        lhs={translate('screens/ConfirmCreateVaultScreen', 'Transaction type')}
        rhs={{
          value:
            props.conversion?.isConversionRequired === true
              ? translate('screens/ConfirmCreateVaultScreen', 'Convert & create vault')
      ...
      />
      <WalletAddressRow />
      <InfoRow type={InfoType.VaultFee} value={vaultFee.toFixed(8)} testID="vault_fee" suffix="DFI" />
...
function SummaryVaultDetails(props: { loanScheme: LoanScheme }): JSX.Element {
  return (
    <>
      <ThemedSectionTitle text={translate('screens/ConfirmCreateVaultScreen', 'VAULT DETAILS')} />
      <NumberRow
