...
export function BorrowMoreScreen({ route, navigation }: Props): JSX.Element {
  ...
  return (
    <ThemedScrollView>
      <ThemedSectionTitle text={translate('screens/BorrowMoreScreen', 'YOU ARE BORROWING MORE FOR LOAN')} />
      ...
      <ThemedSectionTitle text={translate('screens/BorrowMoreScreen', 'VAULT IN USE')} />
      <View style={tailwind('px-4')}>
...
export function VaultInput({
  ...
  const vaultState = useVaultStatus(
    vault.state,
    new BigNumber(vault.collateralRatio),
      ...
      {displayMaxLoanAmount && loanToken !== undefined && (
        <View style={tailwind('flex flex-row items-center justify-between mb-1')}>
          ...
          <NumberFormat
            ...
            displayType="text"
            renderText={(value) => <ThemedText style={tailwind('text-sm font-medium')}>{value}</ThemedText>}
          />
        </View>
      )}
    </ThemedView>
