...
export function EditCollateralScreen({ navigation, route }: Props): JSX.Element {
  ...
  const getTokenAmount = (tokenId: string): BigNumber => {
    ...
    return BigNumber.max(
      new BigNumber(_token === undefined ? 0 : _token.amount).minus(_token?.id === '0_unified' ? reservedDFI : 0),
      0,
    );
  };
    ...
    (collaterals) =>
      collaterals
        ...
        .filter((collateralItem) =>
          new BigNumber(
            getActivePrice(collateralItem.token.symbol, collateralItem.activePrice, collateralItem.factor),
          ).gt(0),
        )
  ...
  return (
    <View style={tailwind('flex-1')} ref={containerRef}>
      <ThemedScrollView contentContainerStyle={tailwind('p-4 pt-0')}>
        ...
        />
        {activeVault.collateralAmounts?.length > 0 && <SectionTitle title="COLLATERAL" />}
      ...
      <BottomSheetWithNav modalRef={bottomSheetRef} screenList={bottomSheetScreen} />
      ...
      {Platform.OS !== 'web' && (
        <BottomSheetWithNav modalRef={bottomSheetRef} screenList={bottomSheetScreen} snapPoints={modalSnapPoints} />
      )}
    </View>
  );
}
    ...
    >
      ...
      >
        <View style={tailwind('flex flex-row items-center')}>
          <SymbolIcon symbol={collateral.displaySymbol} styleProps={tailwind('w-6 h-6')} />
          <ThemedText
...
function CardLabel(props: { text: string }): JSX.Element {
  return (
    <ThemedText light={tailwind('text-gray-500')} dark={tailwind('text-dfxgray-400')} style={tailwind('text-xs mb-1')}>
