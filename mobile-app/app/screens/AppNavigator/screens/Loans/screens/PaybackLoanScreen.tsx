...
export function PaybackLoanScreen({ navigation, route }: Props): JSX.Element {
  ...
  const {
    ...
    const selectedPaymentTokenWithAmount = paymentTokensWithAmount.find(
      (pTokenWithAmount) => pTokenWithAmount.paymentToken.tokenId === selectedPaymentToken.tokenId,
    ) ?? {
    ...
    return {
      ...
      amountToPayInLoanToken,
      hasSufficientPaymentTokenBalance:
        selectedPaymentTokenWithAmount.paymentToken.tokenBalance.gte(amountToPayInPaymentToken),
    ...
    inputToken: {
      ...
      amount: new BigNumber(
        selectedPaymentToken.tokenId === '0_unified'
          ? BigNumber.min(selectedPaymentTokenBalance, amountToPayInPaymentToken.plus(paymentPenalty))
          : 0,
      ),
    },
    ...
    navigation.navigate({
      ...
      params: {
        ...
        excessAmount: isExcess
          ? new BigNumber(amountToPayInPaymentToken).minus(outstandingBalanceInPaymentToken)
          : undefined,
        resultingColRatio,
        paymentPenalty,
        ...
        <View style={tailwind('mt-4 px-4')}>
          <WalletTextInput
            ...
            {...(!hasSufficientPaymentTokenBalance &&
              !isInputEmpty && {
                inlineText: {
                  type: 'error',
                  text: translate('screens/PaybackLoanScreen', 'Insufficient {{token}} to pay for the entered amount', {
          ...
          >
            <>
              <SetAmountButton
                amount={
                  selectedPaymentTokenBalance.gte(outstandingBalanceInPaymentToken)
...
export function LoanTokenInput(props: LoanTokenInputProps): JSX.Element {
  return (
    ...
    >
      <ThemedText light={tailwind('text-gray-400')} dark={tailwind('text-dfxgray-400')} style={tailwind('text-sm')}>
