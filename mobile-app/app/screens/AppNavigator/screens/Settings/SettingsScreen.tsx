...
export function SettingsScreen({ navigation }: Props): JSX.Element {
  ...
  return (
    <ThemedScrollView style={tailwind('flex-1 pb-8')} testID="setting_screen">
      <ThemedSectionTitle testID="network_title" text={translate('screens/Settings', 'GENERAL')} />
      ...
      <ThemedSectionTitle testID="security_title" text={translate('screens/Settings', 'SECURITY')} />
      ...
      {isEncrypted && (
        <>
          <NavigateItemRow label="Recovery Words" onPress={revealRecoveryWords} testID="view_recovery_words" />
          <NavigateItemRow label="Change Passcode" onPress={changePasscode} testID="view_change_passcode" />
        </>
      )}

      <ThemedSectionTitle testID="addtional_options_title" text={translate('screens/Settings', 'ADDITIONAL OPTIONS')} />
...
function SelectedNetworkItem({ network, onPress }: { network: EnvironmentNetwork; onPress: () => void }): JSX.Element {
  return (
    ...
    >
      <ThemedText style={tailwind('font-medium')}>{network}</ThemedText>
  ...
  return (
    <>
      ...
      >
        ...
        <Switch onValueChange={onToggle} value={value} testID="switch_privacy_lock" />
      </ThemedView>
      <ThemedText
...
function NavigateItemRow({
  ...
  return (
    <ThemedTouchableOpacity
      ...
      testID={testID}
    >
      <ThemedText style={tailwind('font-medium')}>{translate('screens/Settings', label)}</ThemedText>
      ...
      <View style={tailwind('flex flex-row items-center')}>
        ...
        <ThemedIcon iconType="MaterialIcons" name="chevron-right" size={24} style={tailwind('mt-0.5')} />
      </View>
    </ThemedTouchableOpacity>
  );
}
