...
export function SettingsScreenV2({ navigation }: Props): JSX.Element {
  ...
  return (
    ...
    >
      <ThemedSectionTitleV2 testID="network_title" text={translate('screens/Settings', 'GENERAL')} />
      ...
      {(isEncrypted || localAuth.isDeviceProtected) && (
        <>
          <ThemedSectionTitleV2 testID="security_title" text={translate('screens/Settings', 'SECURITY')} />
          <ThemedViewV2
            dark={tailwind('bg-mono-dark-v2-00')}
            light={tailwind('bg-mono-light-v2-00')}
            style={tailwind('rounded-lg-v2 pl-5 pr-4')}
...
function NavigateItemRow({ testID, label, value, onPress, border }: INavigateItemRow): JSX.Element {
  return (
    <ThemedViewV2
