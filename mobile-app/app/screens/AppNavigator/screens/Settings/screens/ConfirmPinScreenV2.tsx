...
export function ConfirmPinScreenV2({ route }: Props): JSX.Element {
  ...
  function passcodeChangeSuccess(encrypted: any): void {
    setTimeout(() => {
      setWallet(encrypted).then(() => navigation.dispatch(StackActions.popToTop()));
  ...
  return (
    <ThemedScrollViewV2 style={tailwind('w-full flex-1 flex-col')}>
      ...
      <View style={tailwind('mt-5 px-12')}>
        ...
        {invalid && (
          <Text style={tailwind('text-center font-normal-v2 text-sm text-red-v2')} testID="wrong_passcode_text">
...
function SuccessIndicator(): JSX.Element {
  return (
    <View style={tailwind('flex flex-col items-center my-5')}>
      <MaterialIcons size={36} name="check-circle" iconType="MaterialIcons" style={tailwind('text-green-v2 w-9 h-9')} />
    </View>
  );
}
