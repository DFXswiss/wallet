  ...
  return (
    <ThemedFlatList
      ItemSeparatorComponent={() => (
        <ThemedViewV2 dark={tailwind('bg-dfxblue-900')} light={tailwind('bg-gray-100')} style={tailwind('h-px')} />
      )}
      ListHeaderComponent={
        <ThemedSectionTitleV2
      ...
      renderItem={({ item, index }) => (
        <CommunityItemRow key={item.id} item={item} first={index === 0} last={index === Communities.length - 1} />
      )}
...
const Communities: CommunityItem[] = [
  {
    ...
    url: 'https://github.com/DFXswiss/wallet/issues',
    icon: 'github',
  ...
  },
  {
    ...
    url: 'https://defichain-wiki.com/wiki/DFX_FAQ',
    icon: 'help-circle',
    iconType: 'MaterialCommunityIcons',
  },
  {
...
// last
{
  ...
  return (
    <ThemedTouchableOpacityV2 onPress={handlePress} style={tailwind('flex-row p-4 items-center')} testID={item.id}>
      <ThemedIcon
      ...
      <View style={tailwind('flex flex-grow justify-end items-end')}>
        <ThemedIcon
