  ...
  return (
    <ThemedScrollView light={tailwind('bg-white')} style={tailwind('px-4')}>
      <View style={tailwind('flex-1 items-center justify-center p-4 mt-4 mb-8')}>
        <AppIcon height={100} testID="app_logo" width={150} />
        <ThemedText style={tailwind('text-2xl font-bold')}>{translate('screens/AboutScreen', 'Wallet')}</ThemedText>
        ...
        {hasBetaFeatures && (
          <TouchableOpacity testID="try_beta_features" onPress={() => navigation.navigate('FeatureFlagScreen')}>
      ...
      <View style={tailwind('flex-row justify-center items-center text-center mt-2 mb-6')}>
        {ABOUT_LINKS.map((l) => (
          <TouchableOpacity
            ...
            onPress={async () => await openURL(l.url)}
            key={l.testID}
          >
            <ThemedText style={tailwind('text-sm font-medium')}>{translate('screens/AboutScreen', l.title)}</ThemedText>
          </TouchableOpacity>
        ))}
      </View>
...
function LinkItemRow({ title, subtitle, testID, url, onPress, iconName }: AboutScreenLinks): JSX.Element {
  ...
  return (
    ...
    >
      ...
      <View style={tailwind('flex-col flex-1 ml-4')}>
        <ThemedText dark={tailwind('text-gray-100')} style={tailwind('font-semibold flex-wrap')}>
