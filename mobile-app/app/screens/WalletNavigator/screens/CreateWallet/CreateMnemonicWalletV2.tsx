...
export function CreateMnemonicWalletV2({ navigation }: Props): JSX.Element {
  ...
  return (
    <ThemedScrollViewV2 contentContainerStyle={tailwind('pt-12 px-5 pb-16')} style={tailwind('flex-1')}>
      <View style={tailwind('px-5 mb-12')}>
        <CreateWalletStepIndicatorV2 current={1} steps={CREATE_STEPS} style={tailwind('px-4')} />

        <ThemedTextV2 style={tailwind('text-base mt-7 text-center font-normal-v2')}>
      ...
      >
        {words.length > 0 ? (
          words.map((word, index) => (
            <RecoveryWordRow key={index} index={index} word={word} border={index < words.length - 1} />
          ))
        ) : (
