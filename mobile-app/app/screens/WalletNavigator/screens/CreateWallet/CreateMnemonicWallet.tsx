...
export function CreateMnemonicWallet({ navigation }: Props): JSX.Element {
  ...
  const refreshRecoveryWords = (): void => {
    WalletAlert({
      ...
      message: translate('screens/WalletNavigator', 'You are about to generate a new set of recovery words. Continue?'),
      buttons: [
        {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: (): JSX.Element => (
        <TouchableOpacity onPress={refreshRecoveryWords} testID="reset_recovery_word_button">
          <ThemedIcon
    ...
    navigation.addListener('beforeRemove', (e) => {
      ...
      WalletAlert({
        ...
        message: translate(
          ...
          'If you leave this screen, you will be provided with a new set of 24 recovery words. Do you want to proceed?',
        ),
        buttons: [
          {
  ...
  return (
    <ThemedScrollView dark={tailwind('bg-dfxblue-900')} light={tailwind('bg-white')} style={tailwind('flex-1')}>
      <CreateWalletStepIndicator current={1} steps={CREATE_STEPS} style={tailwind('py-4 px-1')} />
      ...
      {words.length > 0 ? (
        words.map((word, index) => <RecoveryWordRow key={index} index={index} word={word} />)
      ) : (
...
function RecoveryWordRow(props: { index: number; word: string; key: number }): JSX.Element {
  return (
    ...
    >
      <ThemedText style={tailwind('w-12 font-semibold')} testID={`word_${props.index + 1}_number`}>
        {`${props.index + 1}.`}
      </ThemedText>

      <ThemedText style={tailwind('flex-grow font-semibold')} testID={`word_${props.index + 1}`}>
        {props.word}
      </ThemedText>
    </ThemedView>
  );
