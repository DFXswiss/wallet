...
export function LockContextProvider(props: PropsWithChildren<any>): JSX.Element | null {
  ...
  const openCfpVoting = useCallback((): void => {
    ...
    AuthService.Session(false, ApiDomain.LOCK).then(
      async (session) => await openURL(`${paymentUrl}/cfp?token=${session.accessToken ?? ''}`),
    );
  }, []);
  ...
  return <LockKycHookContext.Provider value={context}>{props.children}</LockKycHookContext.Provider>;
}
