...
export function FeatureFlagProvider(props: React.PropsWithChildren<any>): JSX.Element | null {
  ...
  const { data: featureFlags = [], isLoading, isError, refetch } = useGetFeatureFlagsQuery(`${network}.${url}`);
  ...
  function isBetaFeature(featureId: FEATURE_FLAG_ID): boolean {
    return modifiedFeatureFlags.some(
      (flag: FeatureFlag) =>
  ...
  */
  ...
  const context: FeatureFlagContextI = {
    ...
    isBetaFeature,
    isExpertFeature,
    hasBetaFeatures: modifiedFeatureFlags.some(
      (flag) =>
        satisfies(appVersion, flag.version) &&
        flag.networks?.includes(network) &&
