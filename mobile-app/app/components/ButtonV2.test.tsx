...
describe('button', () => {
  buttonFill.forEach((fill) => {
    it(`should match styling of button type ${fill}`, () => {
      ...
      const enabled = render(<ButtonV2 disabled={false} fill={fill} label="Submit" onPress={onPress} />).toJSON();
      expect(enabled).toMatchSnapshot();

      const disabled = render(<ButtonV2 disabled fill={fill} label="Submit" onPress={onPress} />).toJSON();
