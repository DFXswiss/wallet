...
export const BottomSheetFiatPicker = ({
  ...
  fiats,
  paramData,
}: BottomSheetFiatPickerProps): React.MemoExoticComponent<() => JSX.Element> =>
  memo(() => {
    ...
    return (
      <FlatList
        ...
        data={fiats}
        renderItem={({ item }: { item: Fiat }) => <ListItem item={item.name} onPress={() => onFiatPress(item)} />}
      />
    );
  });
    ...
    >
      <View style={tailwind('flex flex-row items-center')}>
        <View style={tailwind('ml-2')}>
          <ThemedText>{props.item}</ThemedText>
        </View>
      </View>
      <View style={tailwind('flex flex-row items-center')}>
