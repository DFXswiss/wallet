...
export function TransactionDetail({ broadcasted, txid, txUrl, onClose, title }: TransactionDetailProps): JSX.Element {
  ...
  return (
    <>
      ...
      <View style={tailwind('flex-auto px-4 justify-center')}>
        <ThemedText style={tailwind('text-sm font-bold')}>{title}</ThemedText>

        {txid !== undefined && txUrl !== undefined && (
          <TransactionIDButton onPress={async () => await gotoExplorer(txUrl)} txid={txid} />
        )}
      </View>
