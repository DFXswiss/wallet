...
export function ButtonV2(props: ButtonProps): JSX.Element {
  const { label, fill = 'fill', styleProps = 'm-4 mt-8' } = props;
  ...
  const buttonText = isLight
    ? `${fill === 'fill' ? 'text-mono-dark-v2-900' : 'text-mono-light-v2-900'} `
    : `${fill === 'fill' ? 'text-mono-light-v2-900' : 'text-mono-dark-v2-900'}`;
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        tailwind(
          `${styleProps} p-3.5 flex-row justify-center ${buttonStyle} ${props.disabled === true ? disabledStyle : ''}`,
        ),
        { borderRadius: 26 },
      ]}
      activeOpacity={0.3}
    >
      <>
        <Text style={tailwind(`${textStyle} font-semibold-v2 text-center`)}>{label}</Text>
      </>
    </TouchableOpacity>
  );
}
