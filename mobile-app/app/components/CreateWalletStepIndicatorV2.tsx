...
export function CreateWalletStepIndicatorV2(props: StepIndicatorProps): JSX.Element {
  ...
  const { current, total, style: containerViewStyle, steps = [], isComplete } = props;
  ...
  function following(): JSX.Element[] {
    ...
    for (let i = 1; i < totalStep; i++) {
      ...
      arr.push(<View key={i * 2} style={tailwind(`h-px flex-grow mt-4 ${lineStyle}`)} />);
      arr.push(
        <StepNode
          content={steps[i]}
          current={current}
  ...
  return (
    <View style={[tailwind('flex-col justify-center items-center w-full'), containerViewStyle]}>
      <View style={tailwind('flex-row justify-center w-10/12 h-16')}>
        <StepNode content={steps[0]} current={current} isLight={isLight} step={1} isComplete={isComplete} />
        {following()}
      </View>
    </View>
  );
...
function StepNode(props: {
  ...
  const { stepperStyle, textStyle } = getStepNodeStyle(props.isLight, props.current, props.step, props.isComplete);
  return (
    <View style={tailwind('flex-col')}>
      <View style={tailwind(`h-9 w-9 rounded-full justify-center items-center relative ${stepperStyle}`)}>
        {props.isComplete === true || props.current > props.step ? (
          ...
          <Text style={tailwind([`${textStyle} font-semibold-v2 text-lg`, { '-mt-1.5': Platform.OS === 'android' }])}>
            {props.step}
          </Text>
        )}
