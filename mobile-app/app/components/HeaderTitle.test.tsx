...
describe('Header title', () => {
    ...
    const component = (
      <Provider store={store}>
        <StoreServiceProvider api={ServiceProviderPersistence}>
          <WhaleProvider>
            <HeaderTitle text="Foo" subHeadingType="Status" onPress={jest.fn()} disabled={false} />
          </WhaleProvider>
  ...
  it('should match snapshot for NetworkSelect', () => {
    const rendered = render(
      <HeaderTitle text="Foo" subHeadingType="NetworkSelect" onPress={jest.fn()} disabled={false} />,
    );
    expect(rendered.toJSON()).toMatchSnapshot();
  });
});
