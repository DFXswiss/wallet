    ...
    buttonColor.forEach((color) => {
      it(`should match styling of button type ${fill}-${color}`, () => {
        ...
        const enabled = render(
          <Button color={color} disabled={false} fill={fill} label="Submit" onPress={onPress} title="Test" />,
        ).toJSON();
        expect(enabled).toMatchSnapshot();

        const disabled = render(
          <Button color={color} disabled fill={fill} label="Submit" onPress={onPress} title="Test" />,
        ).toJSON();
