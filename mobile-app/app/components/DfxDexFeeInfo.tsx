...
export function DfxDexFeeInfo(props: DfxDexFeeInfoProps): JSX.Element {
  ...
  // TODO: use DFX assets to know which assets are considered as Crypto
  const isCrypto = (): boolean =>
    ['DFI', 'dBTC', 'dETH', 'dUSDT', 'dUSDC', 'dLTC', 'dBCH', 'dDOGE', 'dEUROC'].includes(props.token.displaySymbol);
      ...
      {isDexStabilizationEnabled && !isCrypto() /* && dexStabilizationType !== 'none' */ && (
        ...
        >
          <InfoText
            testID="dfx_kyc_info"
            text={translate(
              'components/DfxDexFeeInfo',
              'Please note that all sales of dTokens (e.g.dUSD, dCOIN, dTSLA, dSPY..) are subject to a DEX stabilization fee. The crypto tokens such as DFI, dBTC, dETH and stablecoins such as dUSDT / dUSDC are not affected. Further information',
            )}
            style={props.style}
          />
        </TouchableOpacity>
